...
var (
	...
	format      = flag.String("fmt", "json", "the format of the input data (json, yaml, xml, plist or csv defaults to json)")
...
func main() {
	...
	if *format != "json" && *format != "yaml" && *format != "plist" && *format != "xml" && *format != "csv" {
		flag.Usage()
		fmt.Fprintln(os.Stderr, "fmt must be json, yaml, xml, or plist")
		os.Exit(1)
	}
